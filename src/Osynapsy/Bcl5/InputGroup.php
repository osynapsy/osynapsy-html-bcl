...
class InputGroup extends AbstractComponent
{
    ...
    public function __construct($name, $prefix = null, $postfix = null, $dimension = null)
    {
        ...
        $this->prepend($prefix);        
        $this->textBox = is_object($name) ? $name : $this->textBoxFactory($name);
        $this->append($postfix);
        $this->setDimension($dimension);
    }
    
    protected function textBoxFactory($name)
    {
        $TextBox = new TextBox($name);
        $TextBox->attribute('aria-describedby', $name.'_prefix');
        return $TextBox;
    }
    ...
    {
        $this->getTextBox()->setValue($value);
    }
}
